...
SMOKE_CYCLE_OVERRIDES ?= run.id=smoke_cycle logging.wandb.enabled=false data.selection_ratio=0.5 teacher.candidate_multiplier=2 training.gradient_accumulation_steps=1 training.save_strategy=no training.save_merged_model=false training.merge_smoke_test_required=false
