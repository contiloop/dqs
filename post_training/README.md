...
`post_training/dist/dqs_preference_training_hf/README.md`에 있다.
...
python3 post_training/train_mpo.py --dry-run
```

CPO와 DPO는 각각 별도 run/output/W&B ID를 사용한다.

```bash
python3 post_training/train_cpo.py --dry-run
python3 post_training/train_cpo.py --smoke-step
python3 post_training/train_cpo.py
```

```bash
python3 post_training/train_dpo.py --dry-run
python3 post_training/train_dpo.py --smoke-step
python3 post_training/train_dpo.py
```

1 epoch, effective batch 128이면 mPO/CPO/DPO 모두 41 optimizer step이다
(`ceil(5,200 / 128) = 41`). `configs/cpo_full_response.yaml`과
`configs/dpo_full_response.yaml`의 `freeze_embeddings` 옵션 및
...
python3 post_training/train_mpo.py --smoke-step \
  --set training.freeze_embeddings=true
python3 post_training/train_mpo.py \
  --set training.freeze_embeddings=true
```

```bash
# BF16 CUDA GPU와 2.4 <= torch < 2.11인 image에서 실행
pip install -r post_training/requirements-gpu.txt
python3 post_training/train_mpo.py --dry-run
python3 post_training/train_mpo.py --smoke-step
python3 post_training/train_mpo.py
```
...
```bash
torchrun --nproc_per_node=4 post_training/train_mpo.py --smoke-step
torchrun --nproc_per_node=4 post_training/train_mpo.py
```
...
DPO는
`post_training/outputs/gemma4_e2b_dqs_dpo_full_response_seed42/` 아래에만
`checkpoint-*`, `smoke_step/`, `final/`, `run_manifest.json`을 만들고 최종 모델에는
`dqs_dpo_model.json`을 기록한다. 기존 SFT와 mPO/CPO 출력은 덮어쓰지 않는다.

```text
post_training/outputs/gemma4_e2b_dqs_mpo_setting5_seed42/
...
python3 post_training/eval_mpo.py --profile val

# 최종 test profile
python3 post_training/eval_mpo.py --profile final
```
