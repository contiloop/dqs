...
`post_training/` 자체가 외부 GPU 인스턴스의 실행 루트다. 별도의
`dqs_preference_training_hf/` 중간 디렉터리는 없다. `mPO`, `CPO`, `DPO` 학습에
필요한 runtime은 이 디렉터리의 `configs/`, `src/`, `data/contracts/`, `scripts/`에
있고, 데이터 합성·필터링·감사 자료는 `research/` 아래에 격리되어 있다. SFT
모델과 학습 데이터는 Git에 포함하지 않고 고정된 Hub revision에서 먼저
명시적으로 내려받는다.
...
Git 저장소의 `research/`에는 합성·필터링 코드와 감사 근거가 남지만,
`make build-release`가 만드는 외부 배포 bundle에는 raw golden pairs, source
review, 분석 보고서, 기존 출력과 캐시가 포함되지 않는다. 학습 후 평가는 원래
DQS 평가 파이프라인에서 별도 output 경로로 수행한다.
...
W&B run ID는 기존의 긴 provenance ID를 유지하지만, 로컬 결과 경로는 objective
이름만 사용한다.

- mPO: `outputs/mpo/final`
- CPO: `outputs/cpo/final`
- DPO: `outputs/dpo/final`
- mPO constant-LR: `outputs/mpo_constant/final`
- mPO constant-LR + lambda-SFT 5: `outputs/mpo_lambda5/final`
