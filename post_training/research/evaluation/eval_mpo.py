...
except ImportError:  # Direct execution from the research/evaluation layout.
    from mpo_wandb import (
        MPOWandbConfig,
        initialize_wandb_run,
        require_wandb_runtime_version,
...
DEFAULT_TRAIN_CONFIG = POST_TRAINING_ROOT / "research" / "configs" / "mpo_setting5.yaml"
...
def resolve_eval_paths(
    ...
    *,
    ...
    if not _is_within(run_output, PACKAGE_ROOT):
        raise ValueError(f"post-training run output must stay under {PACKAGE_ROOT}: {run_output}")
