...
class MPOEvalIsolationTest(unittest.TestCase):
    ...
    def test_eval_model_and_output_are_isolated_under_post_training(self) -> None:
        payload = yaml.safe_load(
            (POST_TRAINING_ROOT / "research" / "configs" / "mpo_setting5.yaml").read_text(encoding="utf-8")
        )
        paths = resolve_eval_paths(payload, require_final_artifact=False)
        run_output = (REPO_ROOT / payload["run"]["output_dir"]).resolve()
        self.assertEqual(paths.final_model_dir, run_output / "final")
        self.assertEqual(paths.output_dir, run_output / "eval" / "val")
        self.assertIn(PACKAGE_ROOT.resolve(), paths.output_dir.parents)
    ...
    def test_completed_eval_appends_to_same_wandb_run_at_final_step(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=outputs_root) as temp_dir:
            run_output = Path(temp_dir)
            payload = yaml.safe_load(
                (POST_TRAINING_ROOT / "research" / "configs" / "mpo_setting5.yaml").read_text(
                    encoding="utf-8"
                )
            )
