...
class TrainDPOConfigTest(unittest.TestCase):
    def setUp(self) -> None:
        self.config = yaml.safe_load(
            (ROOT / "configs" / "dpo_full_response.yaml").read_text(encoding="utf-8")
        )

    def test_default_config_is_strict_and_distinct(self) -> None:
        validate_config(self.config)
        dpo_output = _output_dir(self.config["run"], smoke_step=False)
        self.assertTrue(str(dpo_output).startswith(str(ROOT.resolve())))
        cpo = yaml.safe_load(
            (ROOT / "configs" / "cpo_full_response.yaml").read_text(encoding="utf-8")
        )
        mpo = yaml.safe_load(
            (ROOT / "configs" / "mpo_setting5.yaml").read_text(encoding="utf-8")
        )
    ...
    def test_no_truncation_contract_matches_data(self) -> None:
        contract = json.loads(
            (ROOT / "dataset_contract_full_response_preference.json").read_text(
                encoding="utf-8"
            )
        )
