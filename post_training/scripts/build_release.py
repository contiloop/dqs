...
def _ensure_inputs(*, data_mode: str) -> None:
    required_paths = [
        ...
        *(
            DEPLOYMENT_ASSETS / "configs" / filename
            for filename in MPO_VARIANT_CONFIGS
        ),
        *(
...
def _release_config(
    ...
    *,
    ...
    model = payload["model"]
    data = payload["data"]
    run["output_dir"] = OUTPUT_DIRS[objective]
...
def _write_text_files(staging: Path) -> None:
    ...
    shutil.copy2(DEPLOYMENT_ASSETS / ".gitignore", staging / ".gitignore")
