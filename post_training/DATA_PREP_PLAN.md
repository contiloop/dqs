...
`post_training/raw/golden_pairs/subset_000.jsonl`부터 `subset_022.jsonl`까지를
...
|---|---|---|
| mPO | `mpo/train.jsonl` | `post_training/dataset_contract_final_source_filtered.json` |
| CPO | `cpo/train.jsonl` | `post_training/dataset_contract_cpo_full_response.json` |
| DPO | `dpo/train.jsonl` | `post_training/dataset_contract_full_response_preference.json` |

아래 `prepared/`, `raw/`, `source_quality/` 경로는 재생성 시에만 생기는 로컬 산출물이며
현재 repo에는 보존하지 않는다. 최종 contract, 검증 보고서, human adjudication ledger,
샘플 10건만 Git 대상이다.

- builder: `post_training/build_preference_pairs_strict_v2.py`
- builder tests: `post_training/test_build_preference_pairs_strict_v2.py`
- char candidates: `post_training/prepared/preference_candidates_strict_v2.jsonl`
- roundtrip subset: `post_training/prepared/roundtrip_strict_v2.jsonl`
- synthesis ledger: `post_training/analysis/strict_v2_synthesis_rejections.jsonl`
- synthesis summary: `post_training/analysis/strict_v2_synthesis_summary.json`
- raw/char validator: `post_training/validate_preference_pairs_strict_v2.py`
- char validation report: `post_training/analysis/strict_v2_synthesis_validation_report.json`
- token builder: `post_training/build_mpo_token_masks.py`
- strict-v2 source-review input: `post_training/prepared/mpo_tokenized_pairs_strict_v2.jsonl`
- token rejection ledger: `post_training/analysis/strict_v2_mpo_token_mask_rejections.jsonl`
- token summary: `post_training/analysis/strict_v2_mpo_token_mask_summary.json`
- token validator report: `post_training/analysis/strict_v2_mpo_token_mask_validation_report.json`
- final audit: `post_training/audit_strict_v2_dataset.py`,
  `post_training/analysis/strict_v2_final_audit_report.json`
- contract builder: `post_training/build_mpo_dataset_contract.py`
- source review finalizer: `post_training/finalize_preference_datasets.py`
- final decisions: `post_training/source_quality/gpt54mini_source_integrity_v1/final_decisions.jsonl`
- human REVIEW ledger: `post_training/analysis/source_quality_human_adjudications.jsonl`
- final mPO candidates: `post_training/prepared/preference_candidates_final_source_filtered.jsonl`
- final mPO train file: `post_training/prepared/mpo_tokenized_pairs_final_source_filtered.jsonl`
- final mPO contract: `post_training/dataset_contract_final_source_filtered.json`
- full-response DPO/CPO pairs: `post_training/prepared/full_response_preference_pairs_final.jsonl`
- full-response DPO/CPO contract: `post_training/dataset_contract_full_response_preference.json`
- DPO config: `post_training/configs/dpo_full_response.yaml`
- DPO entry point: `post_training/train_dpo.py`
- DPO runtime guards: `post_training/dpo_trainer.py`
- CPO tokenized train file: `post_training/prepared/cpo_tokenized_full_response_pairs_final.jsonl`
- CPO tokenized contract: `post_training/dataset_contract_cpo_full_response.json`
- review samples: `post_training/analysis/STRICT_V2_SAMPLE_10.md`,
  `post_training/analysis/STRICT_V2_MPO_TOKEN_MASK_SAMPLE_10.md`
...
```bash
python3 post_training/build_preference_pairs_strict_v2.py
python3 post_training/validate_preference_pairs_strict_v2.py

python3 post_training/build_mpo_token_masks.py \
  --candidates post_training/prepared/preference_candidates_strict_v2.jsonl \
  --output post_training/prepared/mpo_tokenized_pairs_strict_v2.jsonl \
  --rejections post_training/analysis/strict_v2_mpo_token_mask_rejections.jsonl \
  --summary post_training/analysis/strict_v2_mpo_token_mask_summary.json \
  --sample-jsonl post_training/analysis/strict_v2_mpo_token_mask_sample_10.jsonl \
  --sample-markdown post_training/analysis/STRICT_V2_MPO_TOKEN_MASK_SAMPLE_10.md \
  --sample-ids post_training/analysis/strict_v2_sample_ids.txt \
  --local-files-only

python3 post_training/validate_mpo_token_masks.py \
  --candidates post_training/prepared/preference_candidates_strict_v2.jsonl \
  --tokenized post_training/prepared/mpo_tokenized_pairs_strict_v2.jsonl \
  --rejections post_training/analysis/strict_v2_mpo_token_mask_rejections.jsonl \
  --samples post_training/analysis/strict_v2_mpo_token_mask_sample_10.jsonl \
  --summary post_training/analysis/strict_v2_mpo_token_mask_summary.json \
  --output post_training/analysis/strict_v2_mpo_token_mask_validation_report.json \
  --local-files-only

python3 post_training/build_mpo_dataset_contract.py
python3 post_training/audit_strict_v2_dataset.py
python3 post_training/finalize_preference_datasets.py
python3 post_training/train_mpo.py --config post_training/configs/mpo_setting5.yaml --dry-run
python3 post_training/train_cpo.py --config post_training/configs/cpo_full_response.yaml --dry-run
python3 post_training/train_dpo.py --config post_training/configs/dpo_full_response.yaml --dry-run
python3 -m unittest discover -s post_training -p 'test_*.py' -v
```
